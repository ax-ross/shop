...
class Category extends AppModel
{
    ...
    {
        return R::getAll("SELECT p.*, pd.* FROM product p JOIN product_description pd on p.id = pd.product_id WHERE p.status = 1 AND p.category_id IN ($child_ids) AND pd.language_id = ? LIMIT $start, $perpage", [$lang['id']]);
    }
